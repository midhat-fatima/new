...
style.configure('TButton', font = ('calibri', 20, 'bold'), foreground='darkblue')
style.map("C.TButton",foreground=[('pressed', 'red'), ('active', 'red')],background=[('pressed', '!disabled', 'black'), ('active', 'white')])  
style.map("M.TButton",foreground=[('pressed', 'green'), ('active', 'red')],background=[('pressed', '!disabled', 'green'), ('active', 'grey')])
...
ttk.Button(tk,text="Quit", style="C.TButton",command=ExitApplication).pack(side=sides)

def scramble_gen():
    AfficheGraphique3D = [0] * slen
    for x in range(len(AfficheGraphique3D)):
        AfficheGraphique3D[x] = [0] * 2
    return AfficheGraphique3D
